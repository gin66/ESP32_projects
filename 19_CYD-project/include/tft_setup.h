/*
  ...
  Install the "TFT_eSPI" lbirary by Bodmer to interface with the TFT Display -
  ...
  *** YOU MUST USE THIS User_Setup.h FILE, CHECK FOR THE LATEST VERSION IN THE
  LINK BELOW IN ORDER TO USE THE EXAMPLES FROM RANDOM NERD TUTORIALS ***
...
// #defined TFT_PARALLEL_16_BIT // **** 16 bit parallel ONLY for RP2040
// processor ****
...
// #define TFT_WR PIN_D2       // Write strobe for modified Raspberry Pi TFT
// only
...
#define TFT_RST \
  12  // Set TFT_RST to -1 if display RESET is connected to ESP32 board RST
...
// #define TFT_CS   33  // Chip select control pin (library pulls permanently
// low #define TFT_DC   15  // Data Command control pin - must use a pin in the
// range 0-31 #define TFT_RST  32  // Reset pin, toggles on startup

// #define TFT_WR    4  // Write strobe control pin - must use a pin in the
// range 0-31 #define TFT_RD    2  // Read strobe control pin
...
// 1234567890:-. #define LOAD_FONT8N // Font 8. Alternative to Font 8 above,
// slightly narrower, so 3 digits fit a 160 pixel TFT #define LOAD_GFXFF  //
...
// The parallel interface write cycle period is derived from a division of the
// CPU clock speed so scales with the processor clock. This means that the
// divider ratio may need to be increased when overclocking. I may also need to
// be adjusted dependant on the display controller type (ILI94341, HX8357C etc).
// If RP2040_PIO_CLK_DIV is not defined the library will set default values
// which may not suit your display. The display controller data sheet will
// specify the minimum write cycle period. The controllers often work reliably
// for shorter periods, however if the period is too short the display may not
// initialise or graphics will become corrupted. PIO write cycle frequency =
// (CPU clock/(4 * RP2040_PIO_CLK_DIV))
