...
static int region_code(struct quirc *q, int x, int y) {
  ...
  if (pixel >= QUIRC_PIXEL_REGION) return pixel;

  if (pixel == QUIRC_PIXEL_WHITE) return -1;

  if (q->num_regions >= QUIRC_MAX_REGIONS) return -1;
...
static void find_region_corners(struct quirc *q, int rcode,
...
static void record_capstone(struct quirc *q, int ring, int stone) {
  ...
  if (q->num_capstones >= QUIRC_MAX_CAPSTONES) return;
 ...
 */
static int timing_scan(const struct quirc *q, const struct quirc_point *p0,
                       const struct quirc_point *p1) {
  int n = p1->x - p0->x;
  int d = p1->y - p0->y;
  int x = p0->x;
  ...
  if (p0->x < 0 || p0->y < 0 || p0->x >= q->w || p0->y >= q->h) return -1;
  if (p1->x < 0 || p1->y < 0 || p1->x >= q->w || p1->y >= q->h) return -1;
...
static int fitness_all(const struct quirc *q, int index) {
  ...
  if (version < 0 || version > QUIRC_MAX_VERSION) return score;

  /* Check alignment patterns */
  ap_count = 0;
  while ((ap_count < QUIRC_MAX_ALIGNMENT) && info->apat[ap_count]) ap_count++;
...
static void record_qr_grid(struct quirc *q, int a, int b, int c) {
  ...
  if (q->num_grids >= QUIRC_MAX_GRIDS) return;
   ...
   * transform.
   */
  if (measure_timing_pattern(q, qr_index) < 0) goto fail;
  ...
  if (qr->grid_size > 21) {
    ...
    if (qr->align_region >= 0) {
      ...
      flood_fill_seed(q, reg->seed.x, reg->seed.y, qr->align_region,
                      QUIRC_PIXEL_BLACK, NULL, NULL, 0);
      flood_fill_seed(q, reg->seed.x, reg->seed.y, QUIRC_PIXEL_BLACK,
                      qr->align_region, find_leftmost_to_line, &psd, 0);
    }
  }
...
fail:
   ...
   */
  for (i = 0; i < 3; i++) q->capstones[qr->caps[i]].qr_grid = -1;
  q->num_grids--;
}
