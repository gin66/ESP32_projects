...
class HmPayload {
 public:
  ...
  void enableSerialDebug(bool enable) { mSerialDebug = enable; }

  void addPayloadListener(payloadListenerType cb) { mCbPayload = cb; }
  ...
  void ivSend(Inverter *iv, bool highPrio = false) {
    ...
    if (iv->getDevControlRequest()) {
      ...
      mRadio->sendControlPacket(iv->radioId.u64, iv->devControlCmd,
                                iv->powerLimit, false);
      mPayload[iv->id].txCmd = iv->devControlCmd;
      // iv->clearCmdQueue();
      // iv->enqueCommand<InfoCommand>(SystemConfigPara); // read back power
      // limit
    } else {
      ...
      // DBGHEXLN(cmd);
      mRadio->prepareDevInformCmd(iv->radioId.u64, cmd, mPayload[iv->id].ts,
                                  iv->alarmMesIndex, false);
    ...
    if (p->packet[0] ==
      ...
      if (*pid == 0x00) {
        ...
        if ((*pid & 0x7F) < MAX_PAYLOAD_ENTRIES) {
          memcpy(mPayload[iv->id].data[(*pid & 0x7F) - 1], &p->packet[10],
                 p->len - 11);
        ...
        if ((*pid & ALL_FRAMES) == ALL_FRAMES) {
          ...
          if (((*pid & 0x7f) > mPayload[iv->id].maxPackId) ||
              (MAX_PAYLOAD_ENTRIES == mPayload[iv->id].maxPackId)) {
            mPayload[iv->id].maxPackId = (*pid & 0x7f);
            if (*pid > 0x81) mPayload[iv->id].lastFound = true;
          }
        }
      }
    } else if (p->packet[0] ==
               (TX_REQ_DEVCONTROL +
                ALL_FRAMES)) {  // response from dev control command
      ...
      if ((p->packet[12] == ActivePowerContr) && (p->packet[13] == 0x00)) {
        ...
        iv->clearCmdQueue();
        iv->enqueCommand<InfoCommand>(
            SystemConfigPara);    // read back power limit
        ...
        if (!crcPass && !pyldComplete) {  // payload not complete
          if ((mPayload[iv->id].requested) && (retransmit)) {
            if (mPayload[iv->id].retransmits < mMaxRetrans) {
              ...
              if (iv->devControlCmd == Restart ||
                  iv->devControlCmd == CleanState_LockAndAlarm) {
                // This is required to prevent retransmissions without answer.
                // DPRINTLN(DBG_INFO, F("Prevent retransmit on Restart /
                // CleanState_LockAndAlarm..."));
                ...
                if (false == mPayload[iv->id].gotFragment) {
                  /*
                  DPRINTLN(DBG_WARN, F("nothing received: Request Complete
                  Retransmit")); mPayload[iv->id].txCmd = iv->getQueuedCmd();
                  DPRINTLN(DBG_INFO, F("(#") + String(iv->id) + F(")
                  prepareDevInformCmd 0x") + String(mPayload[iv->id].txCmd,
                  HEX)); mRadio->prepareDevInformCmd(iv->radioId.u64,
                  mPayload[iv->id].txCmd, mPayload[iv->id].ts,
                  iv->alarmMesIndex, true);
                  */
                  // DPRINT_IVID(DBG_INFO, iv->id);
                  // DBGPRINTLN(F("nothing received"));
                  mPayload[iv->id].retransmits = mMaxRetrans;
                } else {
                  for (uint8_t i = 0; i < (mPayload[iv->id].maxPackId - 1);
                       i++) {
                    if (mPayload[iv->id].len[i] == 0) {
                      ...
                      mRadio->sendCmdPacket(iv->radioId.u64, TX_REQ_INFO,
                                            (SINGLE_FRAME + i), true);
          ...
          if (mPayload[iv->id].retransmits < mMaxRetrans) {
            ...
            mRadio->prepareDevInformCmd(iv->radioId.u64, mPayload[iv->id].txCmd,
                                        mPayload[iv->id].ts, iv->alarmMesIndex,
                                        true);
          }
          ...
          for (uint8_t i = 0; i < (mPayload[iv->id].maxPackId); i++) {
            ...
            memcpy(&payload[payloadLen], mPayload[iv->id].data[i],
                   (mPayload[iv->id].len[i]));
          ...
          if (NULL == rec) {
            ...
            if (AlarmData == mPayload[iv->id].txCmd) {
              uint8_t i = 0;
              while (1) {
                if (0 == iv->parseAlarmLog(i++, payload, payloadLen)) break;
                if (NULL != mCbAlarm) (mCbAlarm)(iv);
                yield();
              }
            }
          } else {
  ...
  bool build(uint8_t id, bool *complete) {
    ...
    *complete = true;
    for (uint8_t i = 0; i < mPayload[id].maxPackId; i++) {
      if (mPayload[id].len[i] == 0) *complete = false;
    }
