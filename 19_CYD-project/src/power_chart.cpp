...
static const lv_chart_update_mode_t CHART_UPDATE_MODE =
...
void power_chart_set_chart_obj(lv_obj_t* chart) {
  ...
  // Add series
  chart_series =
      lv_chart_add_series(chart_obj, CHART_COLOR, LV_CHART_AXIS_PRIMARY_Y);
  ...
  // With LV_CHART_UPDATE_MODE_SHIFT, new values will automatically shift old
  // ones
...
void power_chart_update(void) {
  ...
  uint32_t count =
      (buffer_index < POWER_BUFFER_SIZE) ? buffer_index : POWER_BUFFER_SIZE;
  if (count == 0) {
  ...
  // Update chart range
  lv_chart_set_axis_range(chart_obj, LV_CHART_AXIS_PRIMARY_Y,
                          (int32_t)calculated_y_min, (int32_t)calculated_y_max);

  // Get the most recent value
  float latest_power =
      power_buffer[(buffer_index - 1) % POWER_BUFFER_SIZE].power_W;
  ...
  Serial.printf(
      "[PowerChart] Chart updated. Range: %d - %d, Added value: %.1f\n",
      (int32_t)calculated_y_min, (int32_t)calculated_y_max, latest_power);
}

// Get current power value
float power_chart_get_current(void) { return current_power; }

// Get minimum power value (calculated chart range)
float power_chart_get_min(void) { return calculated_y_min; }

// Get maximum power value (calculated chart range)
float power_chart_get_max(void) { return calculated_y_max; }
