...
xy6020l xy(Serial1, 0x01, 50,
           XY6020_OPT_SKIP_SAME_HREG_VALUE | XY6020_OPT_NO_HREG_UPDATE);
...
uint16_t duty = 0;
bool force_off;
enum class XY_State { idle, request, ongoing } xy_current = XY_State::idle;
...
void setup() {
  ...
  Serial1.begin(115200, SERIAL_8N1, 12,
                13);  // 115200 baud, RX=GPIO12, TX=GPIO13
...
void loop() {
  ...
  if ((((int32_t)valid_xy - wait_valid) > 0) &&
      (xy_current == XY_State::idle) && xy.TxBufEmpty()) {
    wait_valid = valid_xy;
    // Over temperature
    if (xy.getTemp() > 600) {
      temp_off = true;
  ...
  if (packetSize == sizeof(struct stromzaehler_packet_s)) {
    struct stromzaehler_packet_s* packet =
        &stromzaehler_packet[(valid_strom + 1) % 3];
    udp2.read((uint8_t*)packet, sizeof(struct stromzaehler_packet_s));
    valid_strom++;
    last_strom_ms = now;
    // Use packet data
    Serial.printf(
        "Time: %02d:%02d:%02d, Weekday: %d, Consumption: %.2f Wh, Production: "
        "%.2f Wh, Current: %.2f W\n",
