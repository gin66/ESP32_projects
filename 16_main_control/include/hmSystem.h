...
const uint8_t MAX_INVERTER = 3;
#define INVERTERTYPE Inverter
 ...
 public:
  ...
  void addInverters(cfgInst_t* config) {
    ...
    for (uint8_t i = 0; i < MAX_NUM_INVERTERS; i++) {
      ...
      //       DBGPRINT("HMT");
      //     else
      //       DBGPRINT(((iv->config->serial.b[4] & 0x03) == 0x01)
      //                    ? " (2nd Gen) "
      //                    : " (3rd Gen) ");
      //
      //     DBGPRINTLN(String(iv->config->serial.u64, HEX));
      //
      //     if ((iv->config->serial.b[5] == 0x10) &&
      //         ((iv->config->serial.b[4] & 0x03) == 0x01))
      //       DPRINTLN(DBG_WARN, F("MI Inverter are not fully supported
      //       now!!!"));
      //   }
      // }
    }
  ...
  INVERTERTYPE* addInverter(cfgIv_t* config) {
    ...
    if ((p->config->serial.b[5] == 0x11) || (p->config->serial.b[5] == 0x10)) {
      ...
      } else if ((p->config->serial.b[4] & 0x03) ==
                 0x02) {  // MI 3rd Gen -> same as HM
        // p->ivGen = IV_HM;
      } else {  // MI 2nd Gen
        // p->ivGen = IV_MI;
      }
  ...
  INVERTERTYPE* getInverterByPos(uint8_t pos, bool check = true) {
    ...
    if (pos >= MAX_INVERTER)
      return NULL;
    else if ((mInverter[pos].initialized &&
              mInverter[pos].config->serial.u64 != 0ULL) ||
             false == check)
