...
static quirc_decode_error_t correct_block(uint8_t *data,
  ...
  if (!block_syndromes(data, ecc->bs, npar, s)) return QUIRC_SUCCESS;

  berlekamp_massey(s, npar, &gf256, sigma);

  /* Compute derivative of sigma */
  memset(sigma_deriv, 0, MAX_POLY);
  for (i = 0; i + 1 < MAX_POLY; i += 2) sigma_deriv[i] = sigma[i + 1];
  ...
  if (block_syndromes(data, ecc->bs, npar, s)) return QUIRC_ERROR_DATA_ECC;
...
static int format_syndromes(uint16_t u, uint8_t *s) {
  ...
  for (i = 0; i < FORMAT_SYNDROMES; i++) {
    ...
    for (j = 0; j < FORMAT_BITS; j++)
      if (u & (1 << j)) s[i] ^= gf16_exp[((i + 1) * j) % 15];
...
static quirc_decode_error_t correct_format(uint16_t *f_ret) {
   ...
   */
  if (!format_syndromes(u, s)) return QUIRC_SUCCESS;

  berlekamp_massey(s, FORMAT_SYNDROMES, &gf16, sigma);

  /* Now, find the roots of the polynomial */
  for (i = 0; i < 15; i++)
    if (!poly_eval(sigma, gf16_exp[15 - i], &gf16)) u ^= (1 << i);

  if (format_syndromes(u, s)) return QUIRC_ERROR_FORMAT_ECC;
...
static void read_data(const struct quirc_code *code, struct quirc_data *data,
  ...
  while (x > 0) {
    ...
    if (!reserved_cell(data->version, y, x)) read_bit(code, data, ds, y, x);
...
static int numeric_tuple(struct quirc_data *data, struct datastream *ds,
  ...
  if (bits_remaining(ds) < bits) return -1;
...
static quirc_decode_error_t decode_numeric(struct quirc_data *data,
  ...
  while (count >= 3) {
    if (numeric_tuple(data, ds, 10, 3) < 0) return QUIRC_ERROR_DATA_UNDERFLOW;
    count -= 3;
  }

  if (count >= 2) {
    if (numeric_tuple(data, ds, 7, 2) < 0) return QUIRC_ERROR_DATA_UNDERFLOW;
    count -= 2;
  }

  if (count) {
    if (numeric_tuple(data, ds, 4, 1) < 0) return QUIRC_ERROR_DATA_UNDERFLOW;
    count--;
  }
...
static quirc_decode_error_t decode_alpha(struct quirc_data *data,
  ...
  while (count >= 2) {
    if (alpha_tuple(data, ds, 11, 2) < 0) return QUIRC_ERROR_DATA_UNDERFLOW;
    count -= 2;
  }

  if (count) {
    if (alpha_tuple(data, ds, 6, 1) < 0) return QUIRC_ERROR_DATA_UNDERFLOW;
    count--;
  }
...
static quirc_decode_error_t decode_byte(struct quirc_data *data,
  ...
  if (data->payload_len + count + 1 > QUIRC_MAX_PAYLOAD)
    return QUIRC_ERROR_DATA_OVERFLOW;
  if (bits_remaining(ds) < count * 8) return QUIRC_ERROR_DATA_UNDERFLOW;
...
static quirc_decode_error_t decode_kanji(struct quirc_data *data,
  ...
  if (data->payload_len + count * 2 + 1 > QUIRC_MAX_PAYLOAD)
    return QUIRC_ERROR_DATA_OVERFLOW;
  if (bits_remaining(ds) < count * 13) return QUIRC_ERROR_DATA_UNDERFLOW;
...
static quirc_decode_error_t decode_eci(struct quirc_data *data,
  ...
  if ((data->eci & 0xc0) == 0x80) {
    ...
    if (bits_remaining(ds) < 16) return QUIRC_ERROR_DATA_UNDERFLOW;
...
static quirc_decode_error_t decode_payload(struct quirc_data *data,
  ...
  if (data->payload_len >= (int)sizeof(data->payload)) data->payload_len--;
...
quirc_decode_error_t quirc_decode(const struct quirc_code *code,
  ...
  if ((code->size - 17) % 4) return QUIRC_ERROR_INVALID_GRID_SIZE;
  ...
  if (err) err = read_format(code, data, 1);
  if (err) return err;
