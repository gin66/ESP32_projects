...
void mpu_read() {
  ...
  i2c.requestFrom(MPU_addr, (uint8_t)14,
                  true);  // request a total of 14 registers
  AcX = i2c.read() << 8 |
        i2c.read();  // 0x3B (ACCEL_XOUT_H) & 0x3C (ACCEL_XOUT_L)
  AcY = i2c.read() << 8 |
        i2c.read();  // 0x3D (ACCEL_YOUT_H) & 0x3E (ACCEL_YOUT_L)
  AcZ = i2c.read() << 8 |
        i2c.read();  // 0x3F (ACCEL_ZOUT_H) & 0x40 (ACCEL_ZOUT_L)
  Tmp = i2c.read() << 8 | i2c.read();  // 0x41 (TEMP_OUT_H) & 0x42 (TEMP_OUT_L)
  GyX =
      i2c.read() << 8 | i2c.read();  // 0x43 (GYRO_XOUT_H) & 0x44 (GYRO_XOUT_L)
  GyY =
      i2c.read() << 8 | i2c.read();  // 0x45 (GYRO_YOUT_H) & 0x46 (GYRO_YOUT_L)
  GyZ =
      i2c.read() << 8 | i2c.read();  // 0x47 (GYRO_ZOUT_H) & 0x48 (GYRO_ZOUT_L)
  ...
  char buf[255];
  sprintf(buf,
          "Acceleration: %6d %6d %6d => %6.1f, Giro: %6d %6d %6d => %6.1f, "
          "Temp: %d",
          AcX, AcY, AcZ, Ac, GyX, GyY, GyZ, Gy, (Tmp + 12420) / 340);
  Serial.println(buf);
}
