...
static float calcMaxPowerAcCh0(Inverter* iv, uint8_t arg0);

static float calcMaxPowerDc(Inverter* iv, uint8_t arg0);
...
class CommandAbstract {
 public:
  ...
  const uint8_t getCmd() { return _Cmd; }
...
const calcFunc_t calcFunctions[] = {
    {CALC_YT_CH0, &calcYieldTotalCh0},   {CALC_YD_CH0, &calcYieldDayCh0},
    {CALC_UDC_CH, &calcUdcCh},           {CALC_PDC_CH0, &calcPowerDcCh0},
    {CALC_EFF_CH0, &calcEffiencyCh0},    {CALC_IRR_CH, &calcIrradiation},
    {CALC_MPAC_CH0, &calcMaxPowerAcCh0}, {CALC_MPDC_CH, &calcMaxPowerDc}};
...
class Inverter {
 public:
  ...
  bool isConnected;       // shows if inverter was successfully identified (fw
  ...
  uint8_t getPosByChFld(uint8_t channel, uint8_t fieldId, record_t* rec) {
    ...
    if (NULL != rec) {
      for (; pos < rec->length; pos++) {
        if ((rec->assign[pos].ch == channel) &&
            (rec->assign[pos].fieldId == fieldId))
          break;
      }
  ...
  void clearDevControlRequest() { mDevControlRequest = false; }
      ...
      if (NULL != rec) {
        if (CMD_CALC != div) {
          ...
          if ((FLD_T == rec->assign[pos].fieldId) ||
              (FLD_Q == rec->assign[pos].fieldId) ||
              (FLD_PF == rec->assign[pos].fieldId)) {
            // temperature, Qvar, and power factor are a signed values
            rec->record[pos] = ((float)((int16_t)val)) / (float)(div);
          } else if (FLD_YT == rec->assign[pos].fieldId) {
            rec->record[pos] =
                ((float)(val) / (float)(div)*generalConfig->yieldEffiency) +
                ((float)config->yieldCor[rec->assign[pos].ch - 1]);
          } else if (FLD_YD == rec->assign[pos].fieldId) {
            rec->record[pos] =
                (float)(val) / (float)(div)*generalConfig->yieldEffiency;
          } else {
            if ((float)(div) > 1)
              rec->record[pos] = (float)(val) / (float)(div);
            else
              rec->record[pos] = (float)(val);
          }
        }
      }
      ...
      if (rec == &recordMeas) {
        ...
        if (getPosByChFld(0, FLD_EVT, rec) == pos) {
          if (alarmMesIndex < rec->record[pos]) {
            ...
            // enqueCommand<InfoCommand>(AlarmUpdate); // What is the function
            // of AlarmUpdate?
  ...
  float getChannelFieldValue(uint8_t channel, uint8_t fieldId, record_t* rec) {
    uint8_t pos = 0;
    if (NULL != rec) {
      for (; pos < rec->length; pos++) {
        if ((rec->assign[pos].ch == channel) &&
  ...
  void doCalculations() {
    ...
    for (uint8_t i = 0; i < rec->length; i++) {
      if (CMD_CALC == rec->assign[i].div) {
        rec->record[i] =
            calcFunctions[rec->assign[i].start].func(this, rec->assign[i].num);
      }
      yield();
    }
  }
  ...
  bool isAvailable() {
    bool avail = false;
    if ((*timestamp - recordMeas.ts) < INVERTER_INACT_THRES_SEC) avail = true;
    if ((*timestamp - recordInfo.ts) < INVERTER_INACT_THRES_SEC) avail = true;
    if ((*timestamp - recordConfig.ts) < INVERTER_INACT_THRES_SEC) avail = true;
    if ((*timestamp - recordAlarm.ts) < INVERTER_INACT_THRES_SEC) avail = true;

    if (avail) {
      if (status < InverterStatus::PRODUCING) status = InverterStatus::STARTING;
    } else {
      if ((*timestamp - recordMeas.ts) > INVERTER_OFF_THRES_SEC) {
        status = InverterStatus::OFF;
        actPowerLimit =
            0xffff;  // power limit will be read once inverter becomes available
      } else
  ...
  bool isProducing() {
    ...
    if (isAvailable()) {
      producing =
          (getChannelFieldValue(CH0, FLD_PAC, &recordMeas) > INACT_PWR_THRESH);
  ...
  record_t* getRecordStruct(uint8_t cmd) {
    switch (cmd) {
      ...
      case InverterDevInform_All:
        return &recordInfo;  //  1 = 0x01
      case SystemConfigPara:
        return &recordConfig;  //  5 = 0x05
      case AlarmData:
        return &recordAlarm;  // 17 = 0x11
      default:
        break;
    }
    return NULL;
  ...
  void initAssignment(record_t* rec, uint8_t cmd) {
    ...
    if (0 != rec->length) {
      rec->record = new float[rec->length];
      memset(rec->record, 0, sizeof(float) * rec->length);
    }
  }
  ...
  uint16_t parseAlarmLog(uint8_t id, uint8_t pyld[], uint8_t len) {
    ...
    start =
        (((uint16_t)pyld[startOff + 4] << 8) | ((uint16_t)pyld[startOff + 5])) +
        startTimeOffset;
    endTime =
        (((uint16_t)pyld[startOff + 6] << 8) | ((uint16_t)pyld[startOff + 7])) +
        endTimeOffset;

    // DPRINTLN(DBG_DEBUG, "Alarm #" + String(pyld[startOff+1]) + " '" +
    // String(getAlarmStr(pyld[startOff+1])) + "' start: " +
    // ah::getTimeStr(start) + ", end: " + ah::getTimeStr(endTime));
...
static float calcEffiencyCh0(Inverter* iv, uint8_t arg0) {
  ...
  if (NULL != iv) {
    ...
    if (dcPower > 0) return acPower / dcPower * 100.0f;
  }
  return 0.0;
}
...
static float calcMaxPowerAcCh0(Inverter* iv, uint8_t arg0) {
  ...
  if (NULL != iv) {
    ...
    if (acPower > acMaxPower) return acPower;
  }
  return acMaxPower;
}
...
static float calcMaxPowerDc(Inverter* iv, uint8_t arg0) {
  ...
  if (NULL != iv) {
    ...
    if (dcPower > dcMaxPower) return dcPower;
  }
  return dcMaxPower;
}
