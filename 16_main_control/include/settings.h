...
class settings {
 public:
  settings() { mLastSaveSucceed = false; }
...
#define LITTLFS_FALSE false
  ...
  void getInfo(uint32_t *used, uint32_t *size) {
    ...
    *size = info.totalBytes;
    ...
    // DPRINTLN(DBG_INFO, String(info.usedBytes) + F(" of ") +
    // String(info.totalBytes)  + F(" used"));
#else
  ...
  bool eraseSettings(bool eraseWifi = false) {
    if (true == eraseWifi) return LittleFS.format();
 ...
 private:
  void loadDefaults(bool keepWifi = false) {
    ...
    mCfg.sys.protectionMask = DEF_PROT_INDEX | DEF_PROT_LIVE | DEF_PROT_SERIAL |
                              DEF_PROT_SETUP | DEF_PROT_UPDATE |
                              DEF_PROT_SYSTEM | DEF_PROT_API | DEF_PROT_MQTT;
...
// restore temp settings
    ...
    if (set) {
      ...
      if (mCfg.sys.protectionMask == 0)
        mCfg.sys.protectionMask =
            DEF_PROT_INDEX | DEF_PROT_LIVE | DEF_PROT_SERIAL | DEF_PROT_SETUP |
  ...
  void jsonInst(JsonObject obj, bool set = false) {
    ...
    JsonArray ivArr;
    if (set) ivArr = obj.createNestedArray(F("iv"));
  ...
  void jsonIv(JsonObject obj, cfgIv_t *cfg, bool set = false) {
    if (set) {
      ...
      uint8_t size = 4;
      if (obj.containsKey(F("pwr"))) size = obj[F("pwr")].size();
