...
UniversalTelegramBot::UniversalTelegramBot(const String& token,
                                           Client& client) {
  updateToken(token);
  this->client = &client;
}

void UniversalTelegramBot::updateToken(const String& token) { _token = token; }

String UniversalTelegramBot::getToken() { return _token; }
...
String UniversalTelegramBot::sendPostToTelegram(const String& command,
                                                JsonObject payload) {
  String body;
  String headers;
...
String UniversalTelegramBot::sendMultipartFormDataToTelegram(
    ...
    const String& fileName, const String& contentType, const String& chat_id,
    int fileSize, MoreDataAvailable moreDataAvailableCallback,
  ...
  if (client->connected()) {
    ...
    if (getNextByteCallback == nullptr) {
      while (moreDataAvailableCallback()) {
        const uint8_t* chunk = getNextBufferCallback();
        size_t to_send = getNextBufferLenCallback();
        while (to_send > 0) {
          size_t sent = client->write(chunk, to_send);
          if (sent < 0) {
#ifdef TELEGRAM_DEBUG
            Serial.println(F("Sending photo from buffer has failure"));
#endif
            closeClient();
            return "";
          } else if (sent == 0) {
#ifdef TELEGRAM_DEBUG
            Serial.println(F("Sent 0 bytes...do not try again"));
#endif
            closeClient();
            return "";
          }
...
bool UniversalTelegramBot::getMe() {
  String response =
      sendGetToTelegram(BOT_CMD("getMe"));  // receive reply from telegram.org
 ...
 * (Argument to pass: Serialied array of BotCommand) * CAUTION: All commands
 *must be lower-case                                      * Returns true, if the
 *command list was updated successfully                    *
 ********************************************************************************/
bool UniversalTelegramBot::setMyCommands(const String& commandArray) {
  ...
  while (millis() - sttime < 8000ul) {  // loop for a while to send the message
    response =
        sendPostToTelegram(BOT_CMD("setMyCommands"), payload.as<JsonObject>());
#ifdef _debug
...
int UniversalTelegramBot::getUpdates(long offset) {
  ...
  if (response == "") {
    ...
    if (!error) {
      ...
      if (response.length() <
          2) {  // Too short a message. Maybe a connection issue
#ifdef TELEGRAM_DEBUG
...
#ifdef TELEGRAM_DEBUG
        Serial.print(
            F("Failed to parse update, the message could be too "
...
bool UniversalTelegramBot::processResult(JsonObject result, int messageIndex) {
  ...
  if (last_message_received != update_id) {
    ...
    if (result.containsKey("message")) {
      ...
      messages[messageIndex].message_id =
          message["message_id"].as<int>();  // added message id
      if (message.containsKey("text")) {
        ...
        if (getFile(messages[messageIndex].file_path,
                    messages[messageIndex].file_size, file_id) == true)
      ...
      if (message.containsKey("reply_to_message")) {
        ...
        // no need to check if containsKey["text"]. If it doesn't, it default to
        // null
      ...
      messages[messageIndex].message_id =
          message["message"]["message_id"].as<int>();  // added message id
 ...
 ***********************************************************************/
bool UniversalTelegramBot::sendSimpleMessage(const String& chat_id,
                                             const String& text,
  ...
  if (text != "") {
    while (millis() - sttime <
           8000ul) {  // loop for a while to send the message
...
bool UniversalTelegramBot::sendMessage(const String& chat_id,
                                       const String& text,
                                       const String& parse_mode,
  ...
  if (message_id != 0) payload["message_id"] = message_id;  // added message_id

  if (parse_mode != "") payload["parse_mode"] = parse_mode;

  return sendPostMessage(
      payload.as<JsonObject>(),
      message_id);  // if message id == 0 then edit is false, else edit is true
}

bool UniversalTelegramBot::sendMessageWithReplyKeyboard(
    const String& chat_id, const String& text, const String& parse_mode,
    const String& keyboard, bool resize, bool oneTime, bool selective) {
  DynamicJsonDocument payload(maxMessageLength);
  payload["chat_id"] = chat_id;
  payload["text"] = text;
  ...
  if (resize) replyMarkup["resize_keyboard"] = resize;

  if (oneTime) replyMarkup["one_time_keyboard"] = oneTime;

  if (selective) replyMarkup["selective"] = selective;

  return sendPostMessage(payload.as<JsonObject>());
}

bool UniversalTelegramBot::sendMessageWithInlineKeyboard(
    const String& chat_id, const String& text, const String& parse_mode,
...
bool UniversalTelegramBot::sendPostMessage(JsonObject payload,
  ...
  if (payload.containsKey("text")) {
    while (millis() < sttime + 8000) {  // loop for a while to send the message
      String response = sendPostToTelegram(
          (edit ? BOT_CMD("editMessageText") : BOT_CMD("sendMessage")),
          payload);  // if edit is true we send a editMessageText CMD
#ifdef TELEGRAM_DEBUG
      Serial.println(response);
#endif
      sent = checkForOkResponse(response);
...
String UniversalTelegramBot::sendPhoto(const String& chat_id,
                                       const String& photo,
...
bool UniversalTelegramBot::checkForOkResponse(const String& response) {
  ...
  return doc["ok"] |
         false;  // default is false, but this is more explicit and clear
}

bool UniversalTelegramBot::sendChatAction(const String& chat_id,
                                          const String& text) {
  bool sent = false;
#ifdef TELEGRAM_DEBUG
...
bool UniversalTelegramBot::getFile(String& file_path, long& file_size,
...
bool UniversalTelegramBot::answerCallbackQuery(const String& query_id,
                                               const String& text,
                                               bool show_alert,
                                               const String& url,
                                               int cache_time) {
