/*
  ...
  - "XPT2046_Touchscreen" library by Paul Stoffregen to use the Touchscreen -
  https://github.com/PaulStoffregen/XPT2046_Touchscreen
  - "TFT_eSPI" library by Bodmer to use the TFT display -
...
//************* software serial pins used for debug (if serial0 is used for
//communication) *************
#define RXPIN 27
#define TXPIN 22
...
// ** Define your vars getter and setter here, if native variables are used in
// your project **
...
void backlight_update() {
  ...
  if (g_backlight_target == BACKLIGHT_FULL &&
      (now - g_last_touch_time > BACKLIGHT_TIMEOUT_MS)) {
    g_backlight_fade_start = g_backlight_current;
  ...
  if (g_backlight_current != g_backlight_target) {
    unsigned long fade_duration = (g_backlight_target == BACKLIGHT_FULL)
                                      ? BACKLIGHT_FADE_UP_MS
                                      : BACKLIGHT_FADE_DOWN_MS;
    ...
    if (elapsed >= fade_duration) {
      ...
      int16_t delta =
          (int16_t)g_backlight_target - (int16_t)g_backlight_fade_start;
      g_backlight_current =
          g_backlight_fade_start + (delta * elapsed / fade_duration);
    }
...
void setup() {
  ...
  // Initialize backlight PWM after TFT init (channel 7 to avoid TFT_eSPI
  // conflicts)
  ledcSetup(7, 5000, 8);
